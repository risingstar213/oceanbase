...
int ObTableSqlService::add_columns_batch_one_trip(common::ObISQLClient &sql_client, const ObTableSchema &table, int64_t start, int64_t length)
{
  ...
  for (int64_t i = 0; (OB_SUCCESS == ret) && (i < length); i++) {
    ObColumnSchemaV2 column = *table.get_column_schema_by_idx(i + start);
    column.set_schema_version(new_schema_version);
    column.set_tenant_id(table.get_tenant_id());
    column.set_table_id(table.get_table_id());
    ObDMLSqlSplicer dml;
...
int ObTableSqlService::add_columns_batch(common::ObISQLClient &sql_client, const ObTableSchema &table)
{
  int ret = OB_SUCCESS;
  const int64_t new_schema_version = table.get_schema_version();
  const uint64_t tenant_id = table.get_tenant_id();
  ...
  int64_t column_num = table.get_column_count();
  int64_t batch_size = 1;
  int64_t offset = 0;
  while (batch_size * 2ll <= column_num) {
    batch_size = batch_size * 2ll;
  }

  while (batch_size != 0 && OB_SUCC(ret)) {
    if (column_num >= batch_size) {
      ret = add_columns_batch_one_trip(sql_client, table, offset, batch_size);
      column_num -= batch_size;
      offset     += batch_size;
    }
    batch_size >>= 1;
  }
                           ...
                           const bool is_truncate_table /* = false */)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    LOG_WARN("get wrong view schema", K(ret), K(table));
             ...
             && table.is_force_view()
             && table.get_column_count() <= 0
             && is_core_table(table.get_table_id())
             && FALSE_IT(table.set_object_status(ObObjectStatus::INVALID))) {
  } else if (table.is_view_table() && data_version >= DATA_VERSION_4_1_0_0
             && table.get_column_count() > 0
             && is_core_table(table.get_table_id())
    ...
    //   LOG_WARN("insert column schema failed, ", K(ret), "table", to_cstring(table));
    // } else 
    if (OB_FAIL(add_constraints(sql_client, table))) {
      LOG_WARN("insert constraint schema failed, ", K(ret), "table", to_cstring(table));
    }
    end_usec = ObTimeUtility::current_time();
    cost_usec = end_usec - start_usec;
